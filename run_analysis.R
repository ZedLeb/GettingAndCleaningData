...
######################################################################  
################### Not part of analysis script ######################  
  
# Write a txt file to upload to Coursera
  write.table(tidy, file = "./Samsung/UCI HAR Dataset/HAR.txt", row.names = FALSE)
  
# check whether the text files reads OK
# from https://class.coursera.org/getdata-009/forum/thread?thread_id=58
  data <- read.table("./Samsung/UCI HAR Dataset/HAR.txt", header = TRUE) 
  View(data)
